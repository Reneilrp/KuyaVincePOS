  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Active Terminals</span>
              ...
              {data.top_products.length === 0 ? (
                ...
                data.top_products.map((prod, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="py-3 px-4 text-sm text-slate-900 dark:text-white font-normal">
                      <span className="text-xs text-slate-500 mr-2 font-mono">{idx + 1}.</span>
                      {prod.product_name}
                    </td>
                    <td className="py-3 px-4 text-center text-sm font-mono text-slate-600 dark:text-slate-300">{prod.total_qty} units</td>
                    <td className="py-3 px-4 text-right text-sm font-mono font-medium text-slate-900 dark:text-white">₱{Number(prod.total_revenue).toFixed(2)}</td>
                  </tr>
                ))
              )}
            </tbody>
