  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 space-y-4 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          <h3 className="text-base font-semibold text-slate-900 dark:text-white flex items-center gap-2">
            <Users className="w-4 h-4 text-slate-400" /> Cashier Roster & PIN Access ({branchStaff.length})
          </h3>
      ...
      <div className="p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-xs text-slate-600 dark:text-slate-400">
        <p className="font-semibold text-slate-800 dark:text-slate-300">Manage staff from Staff & Payroll tab</p>
        <p className="mt-0.5">
          To add cashiers, reset PINs, or update roles, go to <strong className="text-slate-900 dark:text-white">Staff & Payroll</strong> in the sidebar. Changes sync here automatically.
        </p>
      </div>
            ...
            {branchStaff.length === 0 ? (
              ...
              branchStaff.map((staff) => (
                <tr key={staff.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="p-3 text-slate-600 dark:text-slate-400 capitalize">{staff.role}</td>
                  <td className="p-3 font-mono font-medium text-slate-900 dark:text-white">₱{Number(staff.hourly_rate || 85).toFixed(2)}/hr</td>
                  ...
                  <td className="p-3 text-right">
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-slate-50 hover:bg-slate-100 dark:bg-slate-950 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
                    >
